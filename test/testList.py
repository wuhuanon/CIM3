...
##        listmix.ColumnSorterMixin.__init__(self, 4)
        ...
        self.Bind(ULC.EVT_LIST_COL_RIGHT_CLICK, self.OnRightClick, self.list)
        ...
        self.Bind(ULC.EVT_LIST_COL_CHECKING, self.CheckTitle)


    def CheckTitle(self, event):
        self.list.SortItems(self.OnCompareItem)

        event.Skip()

    def OnCompareItem(self, line1, lin2):
        return -1
    ...
    def PopulateList(self):
        ...
        for key, data in items:
            if key == 3:
                index = self.list.InsertImageStringItem(10, data[0], [3, 4, 7], it_kind=1)
            elif key == 4:
                dt = "\n".join(data[0].split())
            ...
            self.list.InsertStringItem(3, "0000")
